...
mask = sga >= 0
s = np.where(mask[119], data[119], 0) #segmentation 있는 부분만 제거 해서 사용하기 위해 변경하는 부분
...
for fid in range(jw_len):
    ...
    tmpmaxx_index = np.where(joints_world[fid,:,0] == joints_world[fid,:,0].max())
    maxx_x, maxx_y, maxx_z = joints_world[fid,tmpmaxx_index][0][0]
    maax, maay = world2pixel(maxx_x, maxx_y, maxx_z)


    tmpminx_index = np.where(joints_world[fid,:,0] == joints_world[fid,:,0].min())
    minx_x, minx_y, minx_z = joints_world[fid,tmpminx_index][0][0]
    miix, miiy = world2pixel(minx_x, minx_y, minx_z)

    tmpmaxy_index = np.where(joints_world[fid,:,1] == joints_world[fid,:,1].max())
    maxy_x, maxy_y, maxy_z = joints_world[fid,tmpmaxy_index][0][0]
    mayyx, mayyy = world2pixel(maxy_x, maxy_y, maxy_z)

    tmpminy_index = np.where(joints_world[fid,:,1] == joints_world[fid,:,1].min())
    miny_x, miny_y, miny_z = joints_world[fid,tmpminy_index][0][0]
    miiyyx, miiyyy = world2pixel(miny_x, miny_y, miny_z)


    x_v = miix, maax, maax, miix, miix
    y_v = mayyy, mayyy, miiyyy,miiyyy,mayyy
